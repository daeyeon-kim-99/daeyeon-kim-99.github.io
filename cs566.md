...
<div class="vertical-stack">
  ...
  <div class="sample-card">
    <img src="/assets/images/sample2(20K)_e50%20.png" alt="Sample 2 — 20K · 50 epochs">
...
- Even though training is extended from 30 to 50 epochs, both the qualitative maps and the metrics in the table show **no dramatic improvement**: loss slightly increases and PCK drops from 0.33 → 0.17.  
- This suggests that, with only **20K images**, the model quickly learns the rough structure but cannot further refine its predictions — the performance is effectively **saturated by dataset size**.  
...
- **Sample 1.** Moving from the 20K subset to full COCO slightly improves the loss  
  (0.003743 → 0.003444) and recovers the PCK from 0.1667 back to **0.3333**.  
...
- **Sample 2.** This harder pose benefits much more from better data and architecture.  
  ...
  Training on full COCO reduces the loss (0.004139 → 0.003892) but PCK still stays at 0.1667,  
  reflecting COCO’s noisy and incomplete lower-body annotations.  
...
- Overall, these numbers confirm the trend we see in the heatmaps:  
  **dataset size** helps (20K → 250K), but **dataset quality + a stronger FPN architecture**  
...
- The enlarged receptive field helps the model understand local spatial structure, so predictions remain relatively stable.  
...
**Sample 2**  
- This more challenging pose exposes the model’s fundamental limitations.  
...
- Increasing the number of training epochs (80 → 160) does not significantly improve performance, meaning the architecture—not training time—is the bottleneck.

**Overall**  
- The dilated CNN demonstrates that a lightweight model can handle simple poses, but it struggles with spatially complex lower-body configurations.  
- These results justify our next step: **introducing self-attention** to better model long-range joint relationships that the CNN alone cannot learn.
