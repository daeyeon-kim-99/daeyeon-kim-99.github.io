...
## 🧠 Model & Heatmap Representation

We use a **heatmap-based model**.  
For each joint, the model outputs a 2D heatmap:

- brighter area → higher confidence that the joint is at that location  
...
> - Increasing epochs from 80 → 160 did **not** significantly improve results.
...
- **Simple dilated CNN**  
  ...
  - but struggles when joints are far apart → **underpowered capacity**.
...
- Modeling **relationships between distant joints** (e.g., hip ↔ ankle) is crucial.
